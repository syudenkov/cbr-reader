...
import { ScrollRenderer } from '../components/viewer/ScrollRenderer';
...
export const ViewerPage = () => {
  ...
  // Only save progress after initial progress has been loaded to prevent overwriting
  useEffect(() => {
    if (fileId && currentPage > 0 && progressLoadingStatus === 'succeeded') {
  ...
  return (
    ...
    >
      ...
      <Box sx={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
        {mode === 'scroll' ? (
          <ScrollRenderer fileId={Number(fileId)} zoomLevel={zoomLevel} />
        ) : (
          <PageRenderer imageUrl={currentPageUrl} zoomLevel={zoomLevel} />
        )}
      </Box>

      {/* Controls - only shown in page mode */}
      {mode === 'page' && <ViewerControls />}
    </Box>
  );
};
